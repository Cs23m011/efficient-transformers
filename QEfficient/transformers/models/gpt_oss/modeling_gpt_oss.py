...
class QEffGptOssMLP(GptOssMLP):
    ...
    def forward(self, hidden_states):
        ...
        expert_in=expert_in.to(gate_proj.device)
        ...
        glu = gate * torch.sigmoid(gate * self.experts.alpha)
        gated_output = (up + 1) * glu
        gated_output=gated_output.to(down_proj.device)
        # Down projection
        experts_out = torch.bmm(gated_output, down_proj) + down_proj_bias.unsqueeze(1)
        experts_out = experts_out.view(bs * seq_len, self.router.top_k, self.experts.hidden_size)
...
class QEffGptOssSlidingDecoderLayer(QEffGptOssDecoderLayer):
    """Marker subclass for sliding-window attention decoder layers.

    Identical behavior to QEffGptOssDecoderLayer but with a distinct class
    identity so that torch.compiler.nested_compile_region (mark_compile_region)
    assigns a different invoke_subgraph identifier, giving each layer type its
    own cached compiled subgraph.  This is required for gpt_oss whose layers
    alternate between sliding-window (ctx_len = sliding_window) and full-context
    (ctx_len = max_cache_len) attention — sharing one graph across both types
    causes shape-constraint failures at export time.
    """


class QEffGptOssFullDecoderLayer(QEffGptOssDecoderLayer):
    """Marker subclass for full-context attention decoder layers.

    See QEffGptOssSlidingDecoderLayer for the rationale.
    """
...
class QEffPrefillOnlyGptOssModel(GptOssModel):
    def __qeff_init__(self):
        ...
        self.sin_cached = torch.nn.Parameter(self.rotary_emb.sin_cached * self.rotary_emb.attention_scaling,requires_grad=False)
        self.cos_cached = torch.nn.Parameter(self.rotary_emb.cos_cached * self.rotary_emb.attention_scaling,requires_grad=False)
...
class QEffGptOssModel(GptOssModel):
    def __qeff_init__(self):
        ...
        # mark_compile_region gives each type a distinct invoke_subgraph identifier.
        # Without this, all layers share one cached graph; gpt_oss alternates
        # sliding-window (ctx_len=128) and full-attention (ctx_len=ctx_len) layers,
        # so a shared graph would have the wrong shape for every other layer.
        layer_types = getattr(self.config, "layer_types", None) or []
        for i, layer in enumerate(self.layers):
            lt = layer_types[i] if i < len(layer_types) else "full_attention"
            if lt == "sliding_attention":
                layer.__class__ = QEffGptOssSlidingDecoderLayer
            else:
                layer.__class__ = QEffGptOssFullDecoderLayer
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # target_length = attention_mask.shape[-1] if isinstance(attention_mask, torch.Tensor) else past_seen_tokens
        causal_mask = _create_causal_mask(position_ids=position_ids, target_length=past_key_values.max_cache_len)
        # Use the actual sliding-window KV cache size (key_cache[0].shape[2]) as the
        # target_length for the sliding mask.  When ctx_len < sliding_window_len (e.g.
        # unit-tests with ctx_len=32 and sliding_window=128) the KV buffer only has
        # ctx_len slots; using the larger config value would mismatch the gather output.
        _sw_cache_len = past_key_values.key_cache[0].shape[2] if past_key_values.key_cache else past_key_values.sliding_window_len
        sliding_mask = _create_causal_mask(
            position_ids=position_ids,
            target_length=_sw_cache_len,
            sliding_window=_sw_cache_len,
        )
